        ...
        <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-8">
          {isSubmitSuccessful ? (
            <div className="text-center">
              ...
              <p className="text-slate-400 text-sm">We sent a password reset link to your email address.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <button type="submit" className="w-full py-3 bg-gradient-to-r from-violet-600 to-blue-600 rounded-xl text-white font-bold hover:from-violet-500 hover:to-blue-500 transition-all">Send Reset Link</button>
            </form>
          )}
          <p className="text-center text-slate-400 text-sm mt-6"><Link href="/login" className="text-violet-400 hover:text-violet-300">← Back to sign in</Link></p>
        </div>
      </div>
    </div>
  )
